      ...
      <div className="container mx-auto px-3 sm:px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
          {services.map((service, index) => (
            <div 
              key={index}
              className="bg-gray-900/50 backdrop-blur-sm p-4 sm:p-6 rounded-xl border border-amber-600/20 hover:border-amber-600/40 transition-all duration-300 hover:-translate-y-1 group"
            >
              <div className={`${service.color} p-2 sm:p-3 rounded-lg w-10 sm:w-14 h-10 sm:h-14 flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300`}>
                <service.icon className="h-5 sm:h-7 w-5 sm:w-7 text-black" />
              </div>
